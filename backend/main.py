...
def init_db():
    ...
    if instructor_count == 0:
        ...
        # 0.7.1 safety: ensure new June wave FIs exist in DB even if table was seeded earlier.
        for name,email,phone,notes in [("Vlad", "", "", "Instructor"), ("Examiner", "", "", "External examiner")]:
            if not conn.execute("SELECT id FROM instructors WHERE LOWER(name)=LOWER(?)", (name,)).fetchone():
                conn.execute("INSERT INTO instructors VALUES (?,?,?,?,?)", (str(uuid.uuid4()), name, email, phone, notes))
    # 0.7.1 safety: ensure June wave students and FIs exist in DB.
    ...
    # 0.7.1 DB DATA MIGRATION: persisted June wave must not keep Lior A.
    ...
    try:
        ...
        if row:
            ...
            if isinstance(flights, list):
                for f in flights:
                    ...
                    if student == "Lior A":
                        if date >= "2026-06-04":
                            ...
                            f["student"] = "Nadav L"
                            changed += 1
                # Apply full 0.7.1 June normalization, including 03 Jun aircraft/FI changes.
                ...
                print(f"✅ 0.7.1 migrated June wave: normalized June schedule", flush=True)
    except Exception as e:
        print(f"⚠️ 0.7.1 June migration skipped: {e}", flush=True)
